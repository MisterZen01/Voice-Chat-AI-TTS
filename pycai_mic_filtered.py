...
# Function to process transcribed text and decide if it should be sent to the character
async def process_text(text, client, chat_session, selected_voice_id, character_id, voice_name):
    """Process transcribed text and send it to the character if it mentions their name."""
    if text:
        print(f"Transcribed text: {text}")

        # Check if the transcribed text contains the character's (voice's) name
        if voice_name.lower() in text.lower():  # Case insensitive match
            ...
            print(f"Text does not mention {voice_name}, not sending to character.")
    else:
...
async def main():
    ...
    try:
        ...
        # Select voice
        voice_name = voice_name  # Set the character's/voice's name here
        ...
        while True:
            ...
            if text:
                ...
                await process_text(text, client, chat_session, selected_voice_id, character_id, voice_name)
